...
#ifndef __AICAPSULE_H__
#define __AICAPSULE_H__
...
public:
	...
	bool GetIntersectionWithLine(const TLineTemp<T>& line) const
	{
		...
		if (abs(f) < 0.999f)
		{
			...
			float dist = plane.GetDistanceTo(start); //异面直线最短距离
			...
			RBOOL bInside = (f0 > 0 && f1 > 0) || (f0 < 0 && f1 < 0);
	...
	bool GetCylinderLineIntersection(TLineTemp<T> &line, T&dist) const
	{
		...
		T hPos = pos.Dot(dir);//投影到cylinder的轴上
