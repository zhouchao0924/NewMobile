...
public:
	...
	TVector3DTemp<T> &ToDirection()
	{
		double fSinH = sin(X*K_G_RAD);  // heading
		double fCosH = cos(X*K_G_RAD);
		double fSinP = sin(Y*K_G_RAD);  // pitch
		double fCosP = cos(Y*K_G_RAD);
