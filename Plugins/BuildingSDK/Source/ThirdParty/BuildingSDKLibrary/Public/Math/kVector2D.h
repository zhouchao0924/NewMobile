...
public:
	...
	bool Equals(const TVector2DTemp<T>& other)const
	{
		return Equals_Float(X, other.X, K_ROUNDING_ERROR) && Equals_Float(Y, other.Y, K_ROUNDING_ERROR);
	}

	//! returns if this vector equals (0,0), taking floating point rounding errors into account
	bool IsZero()const
	{
		return Equals_Float(X, 0, K_ROUNDING_ERROR) && Equals_Float(Y, 0, K_ROUNDING_ERROR);
	}
