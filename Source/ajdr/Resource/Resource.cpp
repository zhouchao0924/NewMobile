...
UTexture *UResource::GetPreviewTexture()
{
	if (!PreviewImage)
	{
		PreviewImage = UResourceUtils::GetImage(this);
		...
		if (!PreviewImage)
		{
			...
			if (Header)
			{
				...
				if (Header->Image.Data.Num() > 0)
				{
					...
					UResourceUtils::SaveImage(this, Header->Image.SizeX, Header->Image.SizeY, Header->Image.Data.GetData(), true);
				}
			}
			else
			{
...
int32 UResource::GetServerVersion()
{
	if (DRInfo)
	{
		int32 SrvVersion = FMath::CeilToInt(DRInfo->GetNumberField(TEXT("version")));
		return SrvVersion;
	}
	return 0;
}
